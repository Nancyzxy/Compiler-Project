...
#line 174 "syntax.tab.c" /* yacc.c:355  */
};
...
{
       0,    47,    47,    53,    54,    56,    57,    60,    61,    64,
      65,    66,    77,    78,    82,    83,    86,    95,    99,   100,
     101,   102,   105,   106,   107,   110,   111,   114,   129,   130,
     133,   134,   137,   138,   139,   140,   141,   142,   143,   144,
     145,   149,   150,   153,   188,   191,   192,   195,   196,   200,
...
#line 66 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode) = insert("ExtDef",3,(yyvsp[-2].treeNode),(yyvsp[-1].treeNode),(yyvsp[0].treeNode));(yyloc) = (yylsp[-2]);(yyval.treeNode)->lineNo=((yylsp[-2])).first_line;
            if(symtab_lookup(root, (yyvsp[-1].treeNode)->child->attribute) != -1){
                printf("Error type 4 at Line %d: redefine function: %s\n",((yylsp[-2])).first_line,(yyvsp[-1].treeNode)->child->attribute);
            }
            else{
              symtab_insert(root, (yyvsp[-1].treeNode)->child->attribute, valType(((yyvsp[-2].treeNode)->child->attribute)));  
            }
...
#line 114 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode) = insert("ParamDec",2,(yyvsp[-1].treeNode),(yyvsp[0].treeNode));(yyloc) = (yylsp[-1]);(yyval.treeNode)->lineNo=((yylsp[-1])).first_line; 
                if( symtab_lookup(root, (yyvsp[0].treeNode)->child->attribute) != -1){
                printf("Error Type 3 at Line %d: redefine variable: %s", ((yylsp[-1])).first_line,(yyvsp[0].treeNode)->child->attribute);
                }
                else{
                        if(strcmp((yyvsp[-1].treeNode)->child->name, "TYPE") == 0){
                            symtab_insert(root, (yyvsp[0].treeNode)->child->attribute, valType(((yyvsp[-1].treeNode)->child->attribute)));
                        }
...
#line 153 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode) = insert("Def",3,(yyvsp[-2].treeNode),(yyvsp[-1].treeNode),alloNodeC(";","SEMI"));(yyloc) = (yylsp[-2]);(yyval.treeNode)->lineNo=((yylsp[-2])).first_line;
        if(strcmp((yyvsp[-1].treeNode)->child->child->child->name, "ID") == 0){
            if(symtab_lookup(root, (yyvsp[-1].treeNode)->child->child->child->attribute) != -1){
            printf("Error Type 3 at Line %d: redefine variable: %s\n", ((yylsp[-2])).first_line,(yyvsp[-1].treeNode)->child->child->child->attribute);
            }
            else{
                if(strcmp((yyvsp[-2].treeNode)->child->name, "TYPE") == 0){
                    symtab_insert(root, (yyvsp[-1].treeNode)->child->child->child->attribute, valType(((yyvsp[-2].treeNode)->child->attribute)));
                    int val = valType(((yyvsp[-2].treeNode)->child->attribute));
                    int cmpVal = 0;
                    if((yyvsp[-1].treeNode)->child->child->next != NULL){
                        if(strcmp((yyvsp[-1].treeNode)->child->child->next->next->child->name,"Exp") == 0){
                            if(strcmp((yyvsp[-1].treeNode)->child->child->next->next->child->child->name, "ID") == 0){
                            cmpVal = symtab_lookup(root, (yyvsp[-1].treeNode)->child->child->next->next->child->child->attribute);
                            if(val != cmpVal){
                            ...
                            cmpVal = valType((yyvsp[-1].treeNode)->child->child->next->next->child->name);
                            if(val != cmpVal){
                                printf("Error type 5 at Line %d: unmatching type on both sides of assignment\n", ((yylsp[-2])).first_line);
                            }
                        }
...
#line 200 "syntax.y" /* yacc.c:1646  */
    {(yyvsp[-1].treeNode) = alloNodeC("=","ASSIGN");(yyval.treeNode)=insert("Exp",3,(yyvsp[-2].treeNode),(yyvsp[-1].treeNode),(yyvsp[0].treeNode));(yyloc) = (yylsp[-2]);(yyval.treeNode)->lineNo=((yylsp[-2])).first_line;
        if(strcmp((yyvsp[-2].treeNode)->child->name, "ID") == 0){
            int val = symtab_lookup(root, (yyvsp[-2].treeNode)->child->attribute);
            int cmpVal = 0;
            if(strcmp((yyvsp[0].treeNode)->child->name, "Exp") == 0){
                if(strcmp((yyvsp[0].treeNode)->child->child->name, "ID") != 0){
                    if(strcmp((yyvsp[0].treeNode)->child->child->name,"float") == 0 || strcmp((yyvsp[0].treeNode)->child->child->name,"int") == 0 || strcmp((yyvsp[0].treeNode)->child->child->name,"char") == 0 ){
                        cmpVal = symtab_lookup(root, (yyvsp[0].treeNode)->child->child->attribute);
                        if(val != cmpVal){
                                printf("Error type 5 at Line %d: unmatching type on both sides of assignment\n", ((yylsp[-2])).first_line);
                        }
                    }
...
#line 239 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode)=insert("Exp",4,alloNodeC((yyvsp[-3].string_value),"ID"),alloNodeC("(","LP"),(yyvsp[-1].treeNode),alloNodeC(")","RP"));(yyloc) = (yylsp[-3]);(yyval.treeNode)->lineNo=((yylsp[-3])).first_line;if(symtab_lookup(root, (yyvsp[-3].string_value)) == -1){printf("Error Type 2 at Line %d: undefined function: %s\n", ((yylsp[-3])).first_line,(yyvsp[-3].string_value));}}
#line 2005 "syntax.tab.c" /* yacc.c:1646  */
    break;

  case 66:
#line 240 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode)=insert("Exp",3,alloNodeC((yyvsp[-2].string_value),"ID"),alloNodeC("(","LP"),alloNodeC(")","RP"));(yyloc) = (yylsp[-2]);(yyval.treeNode)->lineNo=((yylsp[-2])).first_line; if(symtab_lookup(root, (yyvsp[-2].string_value)) == -1){printf("Error Type  2 at Line %d :  function %s is used without definition", ((yylsp[-2])).first_line,(yyvsp[-2].string_value));}}
...
#line 243 "syntax.y" /* yacc.c:1646  */
    {(yyval.treeNode)=insert("Exp",1,alloNodeC((yyvsp[0].string_value),"ID"));(yyloc) = (yylsp[0]);(yyval.treeNode)->lineNo=((yylsp[0])).first_line; if(symtab_lookup(root, (yyvsp[0].string_value)) == -1){printf("Error Type 1 at Line %d: undefined variable: %s\n", ((yylsp[0])).first_line,(yyvsp[0].string_value));} }
